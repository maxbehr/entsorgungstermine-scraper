    ...
    for filename in files:
        ...
        with io.open(f, encoding='utf-8') as data_file:
            ...
            for month_label in m:
                ...
                if len(months[month_label]) > 0:
                    month_obj["year"] = months[month_label][0]["date"]
            ...
            print "Schreibe Gesamtübersicht für " + str(street_name)
            file_name = str(street_id) + " " + str(street_name)
            write_summary(file_name, street)
...
def write_summary(street_name, street_data):
    sum_file = path_summary + street_name + '.json'
    if os.path.exists(sum_file):
        os.remove(sum_file)

    with open(sum_file, 'w') as outfile:
        json.dump(street_data, outfile)
